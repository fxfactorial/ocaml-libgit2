...
module Results = struct
    ...
    type 'a git_success = GIT_OK
                        | GIT_PAYLOAD of 'a

    type 'a git_function_result = Success of 'a git_success
                                | Failure of git_failure
    ...
    let (>>|) left right =
      ...
      | Success x -> match x with
        | GIT_OK | GIT_PAYLOAD _ -> right GIT_OK

    let of_int code =
      match code with
      | -1 -> Failure (GIT_ERROR "Generic Error")
      | -3 -> Failure (GIT_ENOTFOUND "Requested object could not be found")
      | -4 -> Failure (GIT_EEXISTS "Object exists preventing operation")
      | -5 -> Failure (GIT_EAMBIGUOUS "More than one object matches")
      | -6 -> Failure (GIT_EBUFS "Output buffer too short to hold data")
      | -7 ->
         Failure (GIT_EUSER
                "GIT_EUSER is a special error that is never generated by libgit2\
                 ...
                 not by libgit2.")
      | -8 -> Failure (GIT_EBAREREPO "Operation not allowed on bare repository")
      | -9 -> Failure (GIT_EUNBORNBRANCH "HEAD refers to branch with no commits")
      | -10 -> Failure (GIT_EUNMERGED "Merge in progress prevented operation")
      | -11 -> Failure (GIT_ENONFASTFORWARD "Reference was not fast-forwardable")
      | -12 -> Failure (GIT_EINVALIDSPEC "Name/ref spec was not in a valid format")
      | -13 -> Failure (GIT_EMERGECONFLICT "Merge conflicts prevented operation")
      | -14 -> Failure (GIT_ELOCKED "Lock file prevented operation")
      | -15 -> Failure (GIT_EMODIFIED "Reference value does not match expected")
      | -16 -> Failure (GIT_EAUTH "Authentication error")
      | -17 -> Failure (GIT_ECERTIFICATE "Server certificate is invalid")
      | -18 -> Failure (GIT_EAPPLIED "Patch/merge has already been applied")
      | -19 -> Failure (GIT_EPEEL "The requested peel operation is not possible")
      | -20 -> Failure (GIT_EEOF "Unexpected EOF")
      | -30 -> Failure (GIT_PASSTHROUGH "Internal only")
      | -31 -> Failure (GIT_ITEROVER "Signals end of iteration with iterator")
      | _ -> Success GIT_OK
  end
...
let find_repo path =
  ...
  Repository.git_repository_discover (addr root) path 0 None |> of_int >>=
    function
    (* This is ugly *)
    | GIT_OK | GIT_PAYLOAD _ ->
       let result = getf root Buffer.ptr_ in
       let length = getf root Buffer.size_ |> Unsigned.Size_t.to_int in
       Success (GIT_PAYLOAD (string_from_ptr result length))
