...
module Results = struct
    ...
    exception No_payload of string

    type git_error = string * int
    type 'a git_result = Action
                       | Payload of 'a
    ...
    let to_string ((msg, code) : git_error) =
      Printf.sprintf "Error Code: %d Message: %s" code msg

    let get_payload = function
      | Payload a -> a
      | Action -> raise (No_payload "Input was Action, not a payload")
    ...
    let (>>!) x f =
      match x with
      | Ok a -> f (get_payload a)
      | Error _ as x -> x

    let of_int code =
      match code with
      | -1 -> Error ("Generic Error", code)
      | -3 ->  Error ("Requested object could not be found", code)
      | -4 ->  Error ("Object exists preventing operation", code)
      | -5 ->  Error ("More than one object matches", code)
      | -6 ->  Error ("Output buffer too short to hold data", code)
      | -7 ->
         Error
           ("GIT_EUSER is a special error that is never generated by libgit2\
             ...
             not by libgit2.", code)
      | -8 ->  Error ("Operation not allowed on bare repository", code)
      | -9 ->  Error ("HEAD refers to branch with no commits", code)
      | -10 ->  Error ("Merge in progress prevented operation", code)
      | -11 ->  Error ("Reference was not fast-forwardable", code)
      | -12 ->  Error ("Name/ref spec was not in a valid format", code)
      | -13 ->  Error ("Merge conflicts prevented operation", code)
      | -14 ->  Error ("Lock file prevented operation", code)
      | -15 ->  Error ("Reference value does not match expected", code)
      | -16 ->  Error ("Authentication error", code)
      | -17 ->  Error ("Server certificate is invalid", code)
      | -18 ->  Error ("Patch/merge has already been applied", code)
      | -19 ->  Error ("The requested peel operation is not possible", code)
      | -20 ->  Error ("Unexpected EOF", code)
      | -30 ->  Error ("Internal only", code)
      | -31 ->  Error ("Signals end of iteration with iterator", code)
      | _ -> Ok Action
  end
...
let find_repo path =
  ...
  Repository.git_repository_discover (addr root) path 0 None |> of_int >>> fun () ->
  let result = getf root Buffer.ptr_ in
  let length = getf root Buffer.size_ |> Unsigned.Size_t.to_int in
  Ok (Payload (string_from_ptr result length))
